...
from dataset_helper import save_hyperparameters

''' 
   使用.tif结尾的数据集时 使用此训练文件
   无需固定大小，传入训练前会自动进行resize。
   数据集遵循VOC2007格式
   背景的像素点值为0，目标的像素点值为255。这样的数据集可以正常运行但是预测是没有效果的！
   需要改成，背景的像素点值为0，目标的像素点值为1。
   我们用的是私有数据集，诸位可以复制，但是复制时要调参，若有问题可以和我联系
   我们邮箱：19852172095@163.com
...
if __name__ == "__main__":
    ...
    # -----------------------------------8-----------------------------------------------------------------------------------------#
    model_path = "logs\\mobilenet_DenseAspp_100\\best_epoch_weights.pth"
    # ---------------------------------------------------------#
    ...
    #   冻结特定层：选择模型中的一部分层进行冻结。通常，这是模型的前几层或整个骨干网络。
    #
    ...
    # ------------------------------------------------------------------#
    #   root_dataset_path 数据集文件路径  遵循VOC2002格式
    #   dataset_path 选择数据集路径
    #   src_data_path 源图片路径
    #   label_path 标签路径
    # ------------------------------------------------------------------#
    # root_dataset_path = 'VOCdevkit'
    # dataset_path = os.path.join(root_dataset_path,'VOC2007')
    # src_data_path = os.path.join(dataset_path,'JPEGImages')
    # label_path = os.path.join(dataset_path,'SegmentationClass')
    root_dataset_path = 'VOCdevkit'
    dataset_path = os.path.join(root_dataset_path,'VOC2007_flood')
    src_data_path = os.path.join(dataset_path,'JPEGImages')
    label_path = os.path.join(dataset_path,'SegmentationClass')
    # ------------------------------------------------------------------#
    #   建议选项：
    ...
    #   读取数据集对应的txt
    # ---------------------------#
    with open(os.path.join(root_dataset_path, "VOC2007_flood/ImageSets/Segmentation/train.txt"),"r") as f:
        train_lines = f.readlines()
    with open(os.path.join(root_dataset_path, "VOC2007_flood/ImageSets/Segmentation/val.txt"),"r") as f:
    ...
    if True:
        ...
        train_dataset = DeeplabDataset(train_lines, input_shape, num_classes, True, root_dataset_path,src_type,label_type,src_data_path,label_path)
        val_dataset = DeeplabDataset(val_lines, input_shape, num_classes, False, root_dataset_path,src_type,label_type,src_data_path,label_path)
        ...
        # ----------------------#
        if local_rank == 0:
            eval_callback = EvalCallback(model, input_shape, num_classes, val_lines, root_dataset_path, log_dir, Cuda,src_data_path,src_type,label_type,label_path,dataset_path ,eval_flag=eval_flag, period=eval_period)
        else:
            eval_callback = None
        ...
        hyperparameters = {
            ...
            "VOCdevkit_path": root_dataset_path,
            "dice_loss": dice_loss,
            "focal_loss": focal_loss,
            "cls_weights": cls_weights.tolist(),
            "num_workers": num_workers
        }
        save_hyperparameters(hyperparameters, save_dir)
        # ---------------------------------------#
        #   开始模型训练
        # ---------------------------------------#
