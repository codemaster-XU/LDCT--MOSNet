...
def custom_fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, 
    ...
    # =============== 关键优化1：梯度累积技术 =============== 
    accumulation_steps = 2  # 每2步更新一次权重
    optimizer.zero_grad()
    ...
    for iteration, batch in enumerate(gen):
        ...
        if fp16 and scaler is not None:
            with torch.cuda.amp.autocast():
                ...
                # 使用交叉熵损失函数
                loss_value = torch.nn.functional.cross_entropy(outputs, masks)
        else:
            outputs = model_train(images)
            loss_value = torch.nn.functional.cross_entropy(outputs, masks)
        
        # ===== 关键修复：确保loss变量在打印前已被赋值 =====
        ...
        if fp16 and scaler is not None:
            # 缩放损失并反向传播
            loss_value = loss_value / accumulation_steps  # 梯度累积缩放
            ...
            if (iteration + 1) % accumulation_steps == 0:
                ...
                scaler.update()
                optimizer.zero_grad()
                torch.cuda.empty_cache()  # 更新后清理显存
        else:
            # 非混合精度训练
        ...
        # =============== 关键优化4：显存释放 ===============
        del images, masks, outputs, loss_value
        torch.cuda.empty_cache()
        
        # 打印进度（使用已赋值的current_loss）
        if iteration % 100 == 0:
            print(f"Epoch: {epoch+1}/{Epoch}, Iteration: {iteration}/{epoch_step}, Loss: {current_loss:.4f}")
...
class ASPP(nn.Module):
    """标准ASPP模块 (Atrous Spatial Pyramid Pooling)"""
    def __init__(self, in_channels, out_channels=256, rates=[6, 12, 18]):
        ...
        # 三个不同空洞率的空洞卷积分支
        self.conv3x3_1 = self._make_conv(in_channels, out_channels, rates[0])
        self.conv3x3_2 = self._make_conv(in_channels, out_channels, rates[1])
        self.conv3x3_3 = self._make_conv(in_channels, out_channels, rates[2])
    ...
    def _make_conv(self, in_channels, out_channels, dilation):
...
# 使用标准ASPP的DeepLab模型（单层特征复用）
class DeepLab(nn.Module):
    def __init__(self, num_classes, backbone="mobilenet", downsample_factor=16, pretrained=True, input_size=(512, 512)):
        ...
        self.aspp = ASPP(in_channels, 256, 
                         rates=[6, 12, 18] if downsample_factor == 16 else [12, 24, 36])
...
if __name__ == "__main__":
    ...
    batch_size = 16            # 关键优化：从16降为8（减少显存占用）
    src_type = '.png'         # 源图片格式
    label_type = '.png'       # 标签格式
    
    # 输出目录
    output_dir = "deeplabV3（去掉denseASPP为ASPP）-output"
    ...
    num_workers = 8           # 增加num_workers充分利用多核CPU
    Cuda = True
    seed = 11
    distributed = False
    sync_bn = True            # 启用同步批归一化
    fp16 = True               # 启用混合精度训练（RTX 4090完全支持）
    ...
    # 关键修复：使用partial函数解决worker_init_fn参数问题
    worker_init = partial(worker_init_fn, rank=local_rank, seed=seed)
    
    # 数据加载器（优化配置充分利用RTX 4090）
