    ...
    for iteration, batch in enumerate(gen):
        ...
        if fp16 and scaler is not None:
            with torch.cuda.amp.autocast():
                ...
                loss = torch.nn.functional.cross_entropy(outputs, masks.squeeze(1))
        else:
            outputs = model_train(images)
            loss = torch.nn.functional.cross_entropy(outputs, masks.squeeze(1))
...
class DeepLab(nn.Module):
    def __init__(self, num_classes, backbone="mobilenet", downsample_factor=16, pretrained=True):
...
if __name__ == "__main__":
    ...
    batch_size = 16           # 增大batch size充分利用RTX 4090显存
    src_type = '.png'         # 源图片格式
    label_type = '.png'       # 标签格式
