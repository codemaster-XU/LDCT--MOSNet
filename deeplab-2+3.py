...
if __name__ == "__main__":
    ...
    # 输出目录
    output_dir = "deeplabV3（消融模块23）-output"
    os.makedirs(output_dir, exist_ok=True)
    
    # 训练配置
    Init_Epoch = 0
    Freeze_Epoch = 0
    UnFreeze_Epoch = 100      # 总共训练200个epoch
    Freeze_Train = False      # 不使用冻结训练
    
    # 模型配置（仅修改此处：将resnet101改为mobilenet或xception）
    backbone = "mobilenet"    # 修改为支持的backbone类型：mobilenet或xception
    pretrained = True
    model_path = ""
